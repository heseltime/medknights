$('form.box').ready(function(){
	...
	//java/script
	//display answer option .5 p --> cicular, .5 p angular problem
	//problem set: 4-7 pieces
	//soultiuon invisible in analysis
	
	//i think: pre-pop 10 sets
	//hide all but first
	
	
	//1st: setup svg functionality with svg.js
	
	//var draw = SVG().addTo('#polygonsheet').size(600,100).fill('#e8e8e8');
	
	//var poly = 5 + Math.floor(Math.random() * 4); //5 - 8
	
	//var poly_coords = [];
	
	//for (i = 0; i < poly; i++) { 
	//	poly_coords[i] = [Math.floor(Math.random() * 600),  Math.floor(Math.random() * 100)];
	//}

	//var make = draw.polygon([[200,50],[300,25],[500,80]]);
	
	var draw = SVG().addTo('#polygonsheet').size(100,100).fill('#e8e8e8');
	
	var pent = [[25,0],[75,0],[100,50],[75,100],[25,100],[0,50]];
	var make = draw.polygon(pent);
