$('form.box').ready(function(){
	...
	function feedback(thisObj, sub_answer) { //from sub_question, array of 5 answer ids
		...
		answer_pnts = (answer_pnts * 0.2).toFixed(2); 
						
		questions[question] = answer_pnts; //sum for total points
		question++;
		...
		for (i = 0; i < answers_eval.length; i++) {
			if (answers_eval[i]) {
				...
				//could hi lite other answer, but technically redundant
			}
		}
	...
	function next_fn() {
		...
		var answer_corr = $(this).parent('li').find('input').val(); 
		
		var answer_givn_pre = $(this).text();
		
		if (answer_givn_pre == "eher wahrscheinlich") {
			var answer_givn = 1; //html corr.
		} else {
			var answer_givn = 0;
		}
