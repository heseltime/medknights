$('form.box').ready(function(){
	...
	//words for anagrams
	//$.get("/static/js/quiz_wf_words_01.txt", function(data) {
	//	str = data.split('\n');
	...
	$.get("/static/js/quiz_wf_words_01.txt", function(data) {
		...
		var random_an = [];
		var random_w = []; //track anagrams base
		...
		for (i = 0; i < total_qs; i++) {
			selector = Math.floor((Math.random() * str.length));
			while (selector_used.includes(selector)) {
				selector = Math.floor((Math.random() * str.length));
			}
			selector_used.push(selector);
			random_an.push(allanagrams(str[selector]));
			random_w.push(str[selector]);
			
			random_an[i] = random_an[i][Math.floor((Math.random() * random_an[i].length))];
			
			ltr_options[i] = [];
			for (j = 0; j < str[selector].length; j++) {
				if (!ltr_options[i].includes(str[selector][j])) { //chk for duplicates
					ltr_options[i].push(str[selector][j]);
				}
			}
		}
		...
		for (i = 0; i < $('form.box > ul > li').length; i++) {
			...
			for (j = 0; j < 4; j++) { //cycle thry li options == 5
				...
				if (j == random_placement) {
					...
					//alert('k ' + k + '   letter ' + ltr_options[i][k] + '   letters used ' + used_letters + '   out of ' + ltr_options[i]);
				}
	...
	//function foo(callback) {
		...
		//var str = ['letter','word','sentence','paragraph','crossword','nother'];
		//callback(str);
		
		//$.ajax({
		//  url: "/static/js/quiz_wf_words_01.txt",
		//  data: data,
